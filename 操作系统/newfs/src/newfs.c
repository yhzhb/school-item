...
int newfs_mkdir(const char* path, mode_t mode) {
	 //NEWFS_DBG("dood:%d\n",NEWFS_ROUND_UP(NEWFS_ROUND_UP(585*6, UINT32_BITS), NEWFS_BLOCK_SZ())/ NEWFS_BLOCK_SZ());
	printf("信息：\nmax_ino:%d,max_data:%d,map_inode_offset:%d,map_data_offset:%d,inode_offset:%d,data_offset:%d\n",newfs_super.max_ino
     ,newfs_super.max_data,newfs_super.map_inode_offset,newfs_super.map_data_offset, newfs_super.inode_offset,newfs_super.data_offset);
	/* TODO: 解析路径，创建目录 */
	(void)mode;
	boolean is_find, is_root;
	char* fname;
	struct newfs_dentry* last_dentry = newfs_lookup(path, &is_find, &is_root);  //返回上级目录
	struct newfs_dentry* dentry;
	struct newfs_inode*  inode;

	if (is_find) {   //不能创建已有的文件夹
		return -NEWFS_ERROR_EXISTS;
	}

	if (NEWFS_IS_REG(last_dentry->inode)) {  //不能在文件下创建文件夹
...
int newfs_getattr(const char* path, struct stat * newfs_stat) {
	...
	//块大小需要修改
	newfs_stat->st_blksize = NEWFS_BLOCK_SZ();

	if (is_root) {
		newfs_stat->st_size	= newfs_super.sz_usage; 
		newfs_stat->st_blocks = NEWFS_DISK_SZ() / NEWFS_BLOCK_SZ();//块大小需要修改
...
int newfs_readdir(const char * path, void * buf, fuse_fill_dir_t filler, off_t offset,
	...
	if (is_find) {
	    ...
	    if (sub_dentry) {
			//调用filler(buf, fname, NULL, ++offset)表示将fname放入buf中
			//offset++表示挟持读下一级目录
...
int newfs_mknod(const char* path, mode_t mode, dev_t dev) {
	...
	/*if(S_ISDIR(mode)){
		printf("create a file not a dir\n");
	...
	dentry->parent = last_dentry;
	inode = newfs_alloc_inode(dentry);  //为此目录项分配索引节点
	newfs_alloc_dentry(last_dentry->inode, dentry);  //将目录项和索引节点链接起来
...
int main(int argc, char **argv)
{
	...
	newfs_options.device = strdup("/home/students/200111325/ddriver");
