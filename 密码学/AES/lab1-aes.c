...
for(i=0;i<4;i++){
	for(j=0;j<4;j++){
		temp=getNumFromS1Box(array[i][j]);
		array[i][j]=temp;
	}
  }
}
...
//请补充代码
    int real_step = step%4;
    int change_step = 4-real_step;
	leftLoop4int(array,change_step);
...
void mixColumns(int array[4][4])
{
	...
	/*for(i=0;i<4;i++){
	   for(j=0;j<4;j++){
         new_array[i][j] = GFMul(colM[i][0],array[0][j]) ^ GFMul(colM[i][1],array[1][j])
                          ^ GFMul(colM[i][2],array[2][j]) ^ GFMul(colM[i][3], array[3][j]);
	   }
	}*/
	int i,j,k,sum;
	for(i=0;i<4;i++){
      for(k=0;k<4;k++){
            sum =0 ;
	   for(j=0;j<4;j++){
          sum = sum^ GFMul(colM[i][j],array[j][k]);
	   }
        new_array[i][k]=sum;
      }
...
//请补充代码
	...
	for(i=0;i<4;i++){
		for(j=0;j<4;j++){
		   new_array[i][j] = GFMul(deColM[i][0],array[0][j]) ^ GFMul(deColM[i][1],array[1][j])
                          ^ GFMul(deColM[i][2],array[2][j]) ^ GFMul(deColM[i][3], array[3][j]);
		}
  ...
  int i,j,k,sum;
	for(i=0;i<4;i++){
      for(k=0;k<4;k++){
            sum =0 ;
	   for(j=0;j<4;j++){
          sum = sum^ GFMul(deColM[i][j],array[j][k]);
	   }
          new_array[i][k]=sum;
      }
...
void addRoundKey(int array[4][4], int round)
{
 ...
 int i,j;
 for(i=0;i<4;i++){
      splitIntToArray(w[4*round+i],temp_array);

      for(j=0;j<4;j++){
            array[j][i]=array[j][i]^temp_array[j];
      }
 }
...
//请补充代码
  ...
  splitIntToArray(num,warray);//将密钥分为4个字节放入4个数组
  leftLoop4int(warray,1);//将密钥化成的数组循环左移一位，达到字循环（字节移动）的效果
  int i;
  for(i=0;i<4;i++){//经S盒字节替换
         warray[i]=getNumFromSBox(warray[i]);
  }
  res =  mergeArrayToInt(warray)^Rcon[round];//异或
  return res;//得到结果
...
void deAes(char *c, int clen, char *key)
{
    ...
    extendKey(key);//扩展密钥
     ...
     for(k = 0; k < clen; k += 16)
    {
        convertToIntArray(c + k, cArray);//矩阵转换

        addRoundKey(cArray, 10);//一开始的轮密钥加,加密的最后一轮为解密的第一轮
        //int tempArray[4][4];

        for(int i = 9; i > 0; i--)
        {

            deShiftRows(cArray);//逆移位
